...
typedef struct{
    ...
    vector <vector<double> > ccd_open;                        // Custo de instalação de CD's no perido - openning - 100%
    vector <vector<double> > ccd_clos;                        // Custo de instalação de CD's no perido - closing - 30%
    vector <vector<double> > ccd_func;                        // Custo de instalação de CD's no perido - functioning - 10%
...
int main (int argc, char *argv[]){
    ...
    d.typemod = (argc > 3) ? atoi(argv[3]) : 1;       // Model: (1) M model or (2) Mỹz model
    ...
    if (typeBenders == 1){
        //versão annotations
        IloCplex::LongAnnotation decomono = mono.cplex.newLongAnnotation(IloCplex::BendersAnnotation,CPX_BENDERS_MASTERVALUE+1);
        for (IloInt t = 1; t <= d.np; t++){
            for (IloInt j = 1; j <= d.ncd; j++){
                mono.cplex.setAnnotation(decomono, mono.cd_open[d.ncd * (t - 1) + j - 1], CPX_BENDERS_MASTERVALUE);
                mono.cplex.setAnnotation(decomono, mono.cd_clos[d.ncd * (t - 1) + j - 1], CPX_BENDERS_MASTERVALUE);
                if (d.typemod == 1) mono.cplex.setAnnotation(decomono, mono.cd_func[d.ncd * (t - 1) + j - 1], CPX_BENDERS_MASTERVALUE);
            }
            for (IloInt k = 1; k <= d.nf; k++){
                mono.cplex.setAnnotation(decomono, mono.f_open[d.nf * (t - 1) + k - 1], CPX_BENDERS_MASTERVALUE);
                mono.cplex.setAnnotation(decomono, mono.f_clos[d.nf * (t - 1) + k - 1], CPX_BENDERS_MASTERVALUE);
                if (d.typemod == 1) mono.cplex.setAnnotation(decomono, mono.f_func[d.nf * (t - 1) + k - 1], CPX_BENDERS_MASTERVALUE);
            }
        }
    }
    else{
    ...
    char stra[250] = "aux-DMA-CPLEX";
    //char strb[250] = "result-DMA-CPLEX";
    
    if (typeBenders == 1){
        strcat (stra,"-BD-Annot");
        //strcat (strb,"-BD-Annot");
        strcpy(nome3, stra);
        //strcpy(nome2, strb);
    }
    else{
    ...
    if (d.typemod == 1){
        for (int t = 1; t <= d.np; t++){
            for(int i = 1; i <= d.nc; i++){
                for (int k = 1; k <= d.nf; k++){
                    IloExpr r2(env);
                    r2 -= mono.f_func[d.nf * (t - 1) + k - 1];
                    for(int j = 1; j <= d.ncd; j++) r2 += mono.x[d.nc * d.nf * d.ncd * (t - 1) + d.nf * d.ncd * (i - 1) + d.nf * (j - 1) + k - 1];
        ...
        for (int t = 1; t <= d.np; t++){
            for(int i = 1; i <= d.nc; i++){
                for(int j = 1; j <= d.ncd; j++){
                    IloExpr r3(env);
                    r3 -= mono.cd_func[d.ncd * (t - 1) + j - 1];
                    for (int k = 1; k <= d.nf; k++) r3 += mono.x[d.nc * d.nf * d.ncd * (t - 1) + d.nf * d.ncd * (i - 1) + d.nf * (j - 1) + k - 1];
    ...
    if (d.typemod == 2){
        for (int t = 1; t <= d.np; t++){
            for(int i = 1; i <= d.nc; i++){
                for (int k = 1; k <= d.nf; k++){
                    IloExpr r4(env);
                    for (int r = 1; r <= t; r++) r4 -= mono.f_open[d.nf * (r - 1) + k - 1];
                    for (int r = 2; r <= t; r++) r4 += mono.f_clos[d.nf * (r - 1) + k - 1];
                    for(int j = 1; j <= d.ncd; j++) r4 += mono.x[d.nc * d.nf * d.ncd * (t - 1) + d.nf * d.ncd * (i - 1) + d.nf * (j - 1) + k - 1];
        ...
        for (int t = 1; t <= d.np; t++){
            for(int i = 1; i <= d.nc; i++){
                for(int j = 1; j <= d.ncd; j++){
                    IloExpr r5(env);
                    for (int r = 1; r <= t; r++) r5 -= mono.cd_open[d.ncd * (r - 1) + j - 1];
                    for (int r = 2; r <= t; r++) r5 += mono.cd_clos[d.ncd * (r - 1) + j - 1];
                    for (int k = 1; k <= d.nf; k++) r5 += mono.x[d.nc * d.nf * d.ncd * (t - 1) + d.nf * d.ncd * (i - 1) + d.nf * (j - 1) + k - 1];
